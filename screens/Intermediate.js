...
import { level1 } from './intermediateDefaultData' 
...
export default function Intermediate({navigation}) {
    ...
    useEffect(() => {
        ...
        async function getData() {
            const { data } = await supabase.from('category')
                .select(`*, questions(*, options(*)) `).eq('category', 'Intermediate')

             
                const data2 = data?.filter(item => {
                    return item.questions.length == 5 && item.category == "Intermediate" 
                }).map((item, i) => {
    ...
    return (
        <Layout >
            ...
            <Text h3 style={{alignSelf: 'center', marginTop: 50, marginBottom: 25}}>  Intermediate Level</Text> 
                ...
                <ScrollView contentContainerStyle={{width: 'auto'}}> 
                    ...
                    {levels.map((level, i) => (
                        ...
                        <Button 
                            ...
                            onPress={() => {
                                navigation.navigate('IntermediateLevel', {
                                    data: level
                                })
                            }}
                            title={`Level ${i + 1}`}
