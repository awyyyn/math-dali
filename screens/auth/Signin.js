...
export default function Signin() {
	...
	useEffect(() => {
		(async () => {
			const res = await AsyncStorage.getItem("schoolId");
				...
				<View style={styles.container}>
					...
					<View style={{ width: "80%" }}>
						...
						<Input
							...
							cursorColor="#123"
							containerStyle={{ width: "100%" }}
							leftIcon={<Icon name="lock" color="#344E41" />}
							rightIcon={
						...
						<Button
							...
							onPress={async () => {
								...
								if (adminData.school_id != code && adminData.role != "admin") {
									setLoading(false);
									right.value = withSpring(10);
									opacity.value = withSpring(1);
									setError("This admin is not assigned to this school.");
									setTimeout(() => {
										right.value = withSpring(150);
										opacity.value = withSpring(0);
										setError("This admin is not assigned to this school.");
