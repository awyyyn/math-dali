...
export default function Intermediate() {
    ...
    async function getSets () {
        ...
        setLoading(true)
        const { data, error } = await supabase.from('category').select().match({'category': "Intermediate", 'school_id': Number(keys[1][1])})
        if(error){
            alert(error.message)
            setLoading(false)
            return 
                ...
                <ScrollView 
                    ...
                    refreshControl={
                        <RefreshControl 
                            ...
                            onRefresh={async() => {
                                setRefresh(true)
                                const { data, error } = await supabase.from('category').select().match({'category': "Intermediate", 'school_id': Number(schoolInfo?.schoolId)})
                                if(error){
                                    alert(error.message)
                                    setRefresh(false)
                                    return 
